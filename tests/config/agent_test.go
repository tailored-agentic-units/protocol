...
func TestDefaultAgentConfig(t *testing.T) {
	...
	if cfg.Provider.BaseURL != "" {
		t.Errorf("got base_url %q, want empty (provider handles default)", cfg.Provider.BaseURL)
	}
...
func TestDefaultAgentConfig_Format(t *testing.T) {
	cfg := config.DefaultAgentConfig()

	if cfg.Format != "openai" {
		t.Errorf("DefaultAgentConfig format: got %q, want %q", cfg.Format, "openai")
	}
}
...
func TestLoadAgentConfig_MergesWithDefaults(t *testing.T) {
	...
	// BaseURL should be empty (providers auto-construct)
	if cfg.Provider.BaseURL != "" {
		t.Errorf("got base_url %q, want empty (provider handles default)", cfg.Provider.BaseURL)
	}
